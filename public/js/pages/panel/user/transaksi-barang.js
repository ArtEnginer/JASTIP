$(document).ready(function () {
  ...
  // Di dalam fungsi processRegularPayment
  function processRegularPayment(orderData) {
    // Jika metode pembayaran bukan COD, gunakan Midtrans
    if (orderData.paymentMethod !== "cod") {
      processMidtransPayment(orderData);
    } else {
      // Proses COD seperti sebelumnya
      $.ajax({
  ...
  function processMidtransPayment(orderData) {
    ...
    // Kirim ke endpoint Midtrans controller
    $.ajax({
      url: `${API_BASE_URL}/api/midtrans/create-payment`,
      method: "POST",
      contentType: "application/json",
      data: JSON.stringify(midtransData),
      success: function (response) {
        if (response.status === "success") {
          // Buka popup pembayaran Midtrans
          openMidtransPopup(response.snap_token, response.order_id);
        } else {
          showToast("Gagal membuat transaksi pembayaran", "red");
          $(".preloader").hide();
        }
      },
      error: function (xhr, status, error) {
  ...
  // Fungsi untuk membuka popup pembayaran Midtrans
  function openMidtransPopup(snapToken, orderId) {
    // Load script Midtrans jika belum dimuat
    if (typeof snap === "undefined") {
      const script = document.createElement("script");
      script.src = "https://app.sandbox.midtrans.com/snap/snap.js";
      script.setAttribute("data-client-key", "SB-Mid-client-U4g1AdoP3Ny_Oxuh"); // Ganti dengan client key Anda
      script.onload = function () {
        showMidtransPopup(snapToken, orderId);
      };
  ...
  function showMidtransPopup(snapToken, orderId) {
    snap.pay(snapToken, {
      ...
      onClose: function () {
        showToast("Pembayaran dibatalkan", "orange");
        $(".preloader").hide();
      },
    });
  }
  ...
  function handlePaymentSuccess(orderId) {
    showToast("Pembayaran berhasil!", "green");
    cart = [];
    updateCartUI();
    $("#order-number").text(orderId);
    $("#order-total").text(`Rp ${formatCurrency(calculateCartTotal())}`);
    checkoutModal.close();
    orderConfirmModal.open();
  }

  function handlePaymentPending(orderId) {
    showToast("Menunggu pembayaran...", "blue");
    cart = [];
    updateCartUI();
    $("#order-number").text(orderId);
    $("#order-total").text(`Rp ${formatCurrency(calculateCartTotal())}`);
  ...
  function handlePaymentError(orderId) {
    showToast("Pembayaran gagal", "red");
    $(".preloader").hide();
  }
